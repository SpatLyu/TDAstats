...
plot_persist <- function(feature.matrix, flat = FALSE,
  ...
  if (!flat) {
    ...
    g <- ggplot2::ggplot(data = feature.df) +
      ...
      #ggplot2::geom_path(data = df.geompath, ggplot2::aes(x = x, y = y)) +          # reference segment
    ...
    g <- ggplot2::ggplot(data = feature.df) +
      ggplot2::geom_point(ggplot2::aes(x = birth, y = persistence, shape = dimension, colour = dimension)) +
